...
def help():
    print("\n\nUsage: python3 brute-steg.py [b or B] [wrapper file]\n")
    print("The program will create a directory that contains all of the attempted bruteforces.")
    print("The program by default will start at 1 for the interval and offset and go up by powers of two to 1024")
    print("Example: If I want to brute force using bit method with a stegged file named \"wrapped_file\":")
    print("\n\npython3 brute-steg.py b wrapped_file\n")
    exit()
...
def file_creator(bit_or_byte, offset_val, interval_val, folder_name, output):
    ...
    filename = f"{bit_or_byte}_Off({offset_val})_Int({interval_val})"
    ...
    try:
        ...
        if (bit_or_byte == "B" and len(sys.argv) == 3):
            ...
            while offset < 1025:
                interval = 1
                while interval <= 1025:
                    output = byte_extract(wrapper_file, offset, interval)
                    folder_name = "Byte-Method"
                    file_creator("Byte", offset, interval, folder_name, output)
